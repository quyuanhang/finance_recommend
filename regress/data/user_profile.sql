...
select count(*) from
  (
    ...
    inner join 
    (SELECT * from(
        (SELECT id, user_id, org_id from investor2 where is_36kr=0 and is_deleted=0 and org_type=0) as a 
        inner join
        (SELECT id, prefer_industry, prefer_phase, manage_capital from organization2) as b on a.org_id = b.id)
    ) as b on a.id = cast(b.user_id as varchar(20))
    inner join 
    ((SELECT id, attach_cid, industry1, address1, finance_phase FROM company WHERE operation_status in (-1, 0) and finance_phase >= 0 and status = 2)) 
    as c on a.cid = c.id
  ) as d

# 查找tag
select 
uid, group_concat(tag_name) AS tag_name 
from user_crm_tag
group by uid
order by uid
